    ...
    <div class="offer-con">
      <div class="row">
        ...
        <div class="offer-info stack-item col-sm-18">
          ...
          <p class="other">
            <strong>Verificado: {{$offer->confirm_at}}</strong>
              &nbsp;&nbsp;&nbsp;▪&nbsp;&nbsp;&nbsp;
            <span class="end-time">Validade: {{$offer->end}}</span>
          </p>
        </div> <!-- /.offer-info -->
      </div> <!-- /.row -->
    </div> <!-- /.offer-con -->
        ...
        @foreach ($new_offers as $new_offer)
          <li class="stack-item item col-md-6 col-sm-8 col-xs-12" data-link="{{$new_offer->link}}" data-title="{{$new_offer->name}}" data-code="{{$new_offer->code}}" data-type="$new_offer->type" data-img="/images/Store-{{$new_offer->store_id}}.png" data-name="{{$new_offer->store_name}}" data-id="{{$new_offer->id}}" data-rule="{{$new_offer->description}}" data-time-verify="{{$new_offer->confirm_at}}" data-time-expire="{{$new_offer->end}}">
            <div class="stack-item-con">
              ...
              <p class="offer-title">
                <a class="offer-title-con offer-popup" href="{{ url('/cupons/'.$new_offer->store_id.'/'.$new_offer->id) }}" title="Cupom de Desconto {{$new_offer->store_name}}" target="_blank">{{$new_offer->name}}</a>
                <span class="blank-empty"></span>
              </p>
              <p>
              @if ($new_offer->type =='C')
                <a class="offer-popup btn btn-info" href="{{ url('/cupons/'.$new_offer->store_id.'/'.$new_offer->id) }}" title="Cupom de {{$new_offer->store_name}}" target="_blank">Pegar cupom</a>
              @else
                <a class="offer-nocode offer-popup btn btn-info" href="{{ url('/cupons/'.$new_offer->store_id.'/'.$new_offer->id) }}" title="Cupom de Desconto {{$new_offer->store_name}}" target="_blank">Compre agora</a>
              @endif
              </p>
